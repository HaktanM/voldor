...
class VOLDOR_SLAM:
    ...
    def __init__(self, mode='mono', path_to_data=None):
        
        # Set the number of flows
        self.voldor_winsize = 5
        
        # Create our datahandler
        self.data = my_utils.dataHandler(path_to_data)
        
        # Get Our Camera Model
        self.cam_model = my_utils._CameraModel()
        
        # Our gravity vector
        self.gravity = np.array([0.0, 0.0, 9.80665016174316]).reshape(3,1)
        
        # Get the width and height of the flows
        path_to_first_flow = self.data.FlowList.getItemPath(0)
        flow_example = my_utils.load_flow(path_to_first_flow)
        self.N_FRAMES = self.data.FlowList.itemCount()+1
        self.h = int(flow_example.shape[0])
        self.w = int(flow_example.shape[1])
        ...
        self.depth_scaling_conf_thresh = 0.5
    ...
    def flow_loader(self, resize=1.0, n_cache=100, flow_range=(-1,-1)):
        self.flow_loader_pt = 0

        path_flow_example = self.data.FlowList.getItemPath(0)
        flow_example = load_flow(path_flow_example)
        
        self.N_FRAMES = self.data.FlowList.itemCount()
        self.h = int(flow_example.shape[0]*resize)
        self.w = int(flow_example.shape[1]*resize)
        
        if flow_range == (-1,-1):
            flow_range = (0,self.N_FRAMES)
        
        for flow_idx in range(flow_range[0], flow_range[1]):
            
            # We wait if enough flow is already loaded
            while len(self.flows) - self.fid_cur > n_cache:
                time.sleep(0.01)

            path_to_flow = self.data.FlowList.getItemPath(flow_idx)
    ...
    def process_vo(self):
        ...
        with self._map_lock.r_locked():
            ...
            curr_timestamp = self.data.FlowList.getItemID(self.fid_cur)
            next_timestamp = self.data.FlowList.getItemID(self.fid_cur+1)
            
            Delta_R, Delta_v, Delta_t = self.data.get_preintegrated_measurement(curr_timestamp=curr_timestamp, next_timestamp=next_timestamp)
            R_b1_b2 = Delta_R.transpose()
            R_c1_c2 = self.cam_model.R_b_c @ R_b1_b2 @ self.cam_model.R_c_b
            
            
            
            for idx in range(self.voldor_winsize):  
                # Get the previous pose        
                prev_timestamp = self.data.FlowList.getItemID(self.fid_cur+idx-0)
                q_bp_g, v_bp_g, t_bp_g = self.data.get_gt(prev_timestamp)
                
                R_bp_g = R.from_quat(q_bp_g).as_matrix().reshape(3,3)
                t_bp_g = t_bp_g.reshape(3)
                T_bp_g = np.eye(4, dtype=np.float32)
                T_bp_g[:3,:3] = R_bp_g
                T_bp_g[:3,3] = t_bp_g
                T_c0_g = T_bp_g @ self.cam_model.T_c_b
                
                # Get the current pose (now) pose 
                curr_timestamp = self.data.FlowList.getItemID(self.fid_cur+idx+1)
                q_bn_g, v_bn_g, t_bn_g = self.data.get_gt(curr_timestamp)
                
                R_bn_g = R.from_quat(q_bn_g).as_matrix().reshape(3,3)
                t_bn_g = t_bn_g.reshape(3)
                T_bn_g = np.eye(4, dtype=np.float32)
                T_bn_g[:3,:3] = R_bn_g
                T_bn_g[:3,3] = t_bn_g
                T_c1_g = T_bn_g @ self.cam_model.T_c_b
                
                # Get the incremental transformation
                T_g_c1 = np.linalg.inv(T_c1_g)
                T_c0_c1 = T_g_c1 @ T_c0_g
                
                t_c0_c1 = T_c0_c1[:3,3]
                # print(f"prev_timestamp : \n{prev_timestamp}")
                print(f"t_c0_c1 : \n{t_c0_c1}")

                depth_prior_poses.append(T44_to_T6(T_c0_c1))
                # depth_priors.append(self.depths[self.fid_cur-1])
