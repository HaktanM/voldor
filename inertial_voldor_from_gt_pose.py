...
if __name__ == '__main__':
    ...
    for flow_idx in range(50, num_of_flows-slam.voldor_winsize-1):
        ...
        for win_idx in range(slam.voldor_winsize):
            ...
            t_b0_g = t_b0_g.reshape(3)
            T_b0_g = np.eye(4, dtype=np.float32)
            T_b0_g[:3,:3] = R_b0_g
            T_b0_g[:3,3] = t_b0_g
            T_c0_g = T_b0_g @ slam.cam_model.T_c_b

            # Next GT
            next_flow_timestamp = slam.data.FlowList.getItemID(flow_idx + win_idx + 1)
            q_b1_g, v_b1_g, t_b1_g = slam.data.get_gt(next_flow_timestamp) 
            
            R_b1_g = R.from_quat(q_b1_g).as_matrix().reshape(3,3)
            t_b1_g = t_b1_g.reshape(3)
            T_b1_g = np.eye(4, dtype=np.float32)
            T_b1_g[:3,:3] = R_b1_g
            T_b1_g[:3,3] = t_b1_g
            T_c1_g = T_b1_g @ slam.cam_model.T_c_b
        ...
        kinematic_state = np.concatenate((R_b0_g, v_gb_g, slam.cam_model.T_c_b[:3,:].astype(np.float32).reshape(3,4)), axis=1)
